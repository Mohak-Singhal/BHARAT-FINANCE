// Shared LLM (Groq) helpers used by /api/chat, /api/ai-coach and
// /api/ai/finance-coach/* serverless routes. All logic runs server-side.
...
export function getSystemPrompts(language: string) {
  const basePrompt = `
CORE PHILOSOPHY:
...
// Calls Groq with automatic model fallback. Throws if no key configured.
export async function getGroqCompletion(
  ...
  const language = preferredLanguage || detectLanguage(messages[messages.length - 1]?.content || '')
  const payload = [
    ...getSystemPrompts(language),
  ...
  for (const model of GROQ_MODELS) {
    try {
      ...
      if (response.ok) {
        ...
        if (content) {
          return { response: cleanTextForTTS(content), model }
        }
      }
    } catch (error) {
      lastError = error
  ...
  throw lastError instanceof Error ? lastError : new Error('Groq completion failed')
}